...
describe('lib/assert/is-type', () => {
	let AssertionError;
	let assertIsType;
	let isType;

	beforeEach(() => {
		isType = sinon.stub().returns(true);
		mockery.registerMock('../verify/is-type', isType);
		AssertionError = require('../../../../lib/error/assertion-error');
		assertIsType = require('../../../../lib/assert/is-type');
	});

	it('exports a function', () => {
		assert.isFunction(assertIsType);
	});

	describe('assertIsType(value, type, message)', () => {
		let caughtError;

		beforeEach(() => {
			try {
				assertIsType('mock-value', 'mock-type', 'mock-message');
			} catch (error) {
				caughtError = error;
			}
		});
		...
		describe('when the value\'s type is not equal to type', () => {
			...
			describe('and no message is provided', () => {
				...
				beforeEach(() => {
					try {
						assertIsType('mock-value', 'mock-type');
					} catch (error) {
						caughtError = error;
					}
				});
				...
				it('throws an AssertionError with a default message', () => {
					assert.isInstanceOf(caughtError, AssertionError);
					assert.strictEqual(caughtError.message, 'Expected "mock-value" to be of type mock-type');
				});
