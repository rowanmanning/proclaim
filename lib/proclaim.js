...
(function(undef) {
	...
	proclaim.isNaN = function(val, msg) {
		proclaim.isNumber(val, 'number', msg);
		proclaim.notStrictEqual(val, val, msg);
	};

	// Assert that a value is not NaN
	proclaim.isNotNaN = function(val, msg) {
		proclaim.isNumber(val, 'number', msg);
		proclaim.strictEqual(val, val, msg);
	};
	...
	proclaim.hasName = function(fn, expected, msg) {
		if (functionsHaveNames) {
			...
			proclaim.equal(
				Function.prototype.toString.call(fn).match(/function\s*([^\s]*)\s*\(/)[1],
				expected,
				msg
			);
		}
	...
	proclaim.arity = function(fn, expected, msg) {
		...
		proclaim.lengthEquals(fn, expected, msg);
	};
	...
	proclaim.almostEqual = function(actual, expected, precision, msg) {
		...
		tolerance = 0.5 * Math.pow(10, -precision);
	...
	function objectsEqual(obj1, obj2, strict) {
		...
		var key;
		var len;
		var obj1IsArgumentsObject;
		var obj1Keys;
		var obj1prototype;
		var obj2IsArgumentsObject;
		var obj2Keys;
		var obj2prototype;
		...
		if (strict) {
			...
			obj1prototype = obj1.prototype || (obj1).__proto__;
			obj2prototype = obj2.prototype || (obj2).__proto__;
		...
		obj1IsArgumentsObject = isArgumentsObject(obj1);
		obj2IsArgumentsObject = isArgumentsObject(obj2);
		if (
	...
	function functionThrows(fn, expected, shouldThrow) {
		...
		isUnexpectedException = !shouldThrow && thrownError && !expected;
		isUnwantedException = !shouldThrow && isError(thrownError);
		...
		if (
			(
				...
				thrownError &&
				expected &&
				!errorMatches(thrownError, expected)
			) || (
				!shouldThrow &&
				thrownError
			)
